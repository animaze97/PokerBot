...
def batch_creator(batch_size):
    ...
    batch_x = [trainx[k:k + batch_size] for k in xrange(0,  1918, batch_size)]
    batch_y = [trainy[k:k + batch_size] for k in xrange(0,  1918, batch_size)]
...
hidden_layer4 = tf.nn.sigmoid(hidden_layer4)
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        avg_cost = 0
        total_batch = int( 1918 / batch_size)
